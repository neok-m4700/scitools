    ...
    if x is None and y is None and z is None:
        ...
        if indexing == 'ij':
            assert (
                ...
                shape(x) == (nx, 1, 1) or
                shape(x) == (nx,)
            ), '_check_xyzv: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny, nz), (nx, 1, 1), (nx,))

            if shape(x) == (nx, ny, nz):
                assert shape(y) == (nx, ny, nz), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (nx, ny, nz), shape(y))
                assert shape(z) == (nx, ny, nz), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (nx, ny, nz), shape(z))
            elif shape(x) == (nx, 1, 1):
                assert shape(y) == (1, ny, 1), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (1, ny, 1), shape(y))
                assert shape(z) == (1, 1, nz), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (1, 1, nz), shape(z))
            else:  # shape(x) == (nx,)
                assert shape(y) == (ny,), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (ny,), shape(y))
                assert shape(z) == (nz,), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (nz,), shape(z))
        else:
            assert (
                shape(x) == (nx, ny, nz) or
                shape(x) == (1, ny, 1) or
                shape(x) == (ny,)
            ), '_check_xyzv: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny, nz), (1, ny, 1), (ny,))

            if shape(x) == (nx, ny, nz):
                assert shape(y) == (nx, ny, nz), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (nx, ny, nz), shape(y))
                assert shape(z) == (nx, ny, nz), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (nx, ny, nz), shape(z))
            elif shape(x) == (1, ny, 1):
                assert shape(y) == (nx, 1, 1), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (nx, 1, 1), shape(y))
                assert shape(z) == (1, 1, nz), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (1, 1, nz), shape(z))
            else:  # shape(x) == (ny,)
                assert shape(y) == (nx,), '_check_xyzv: x has shape %s, expected y to be %s, not %s' % (shape(x), (nx,), shape(y))
                assert shape(z) == (nz,), '_check_xyzv: x has shape %s, expected z to be %s, not %s' % (shape(x), (nz,), shape(z))
    ...
    if x is None and y is None:
        ...
        if indexing == 'ij':
            assert (
                ...
                shape(x) == (nx, 1) or
                len(x) == nx
            ), '_check_xyz: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny), (nx, 1), (nx,))

            assert (
                shape(y) == (nx, ny) or
                shape(y) == (1, ny) or
                len(y) == ny
            ), '_check_xyz: y has shape %s, expected %s, %s, or %s' % (shape(y), (nx, ny), (1, ny), (ny,))
        else:
            assert (
                shape(x) == (nx, ny) or
                shape(x) == (1, ny) or
                len(x) == ny
            ), '_check_xyz: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny), (1, ny), (ny,))

            assert (
                shape(y) == (nx, ny) or
                shape(y) == (nx, 1) or
                len(y) == nx
            ), '_check_xyz: y has shape %s, expected %s, %s, or %s' % (shape(y), (nx, ny), (nx, 1), (nx,))
...
def _check_xyuv(*args, **kwargs):
    ...
    if len(us) == 1:
        if x is None and y is None:
            ...
            assert shape(x) == us, '_check_xyuv: x has shape %s, expected %s' % (shape(x), us)
            assert shape(y) == us, '_check_xyuv: y has shape %s, expected %s' % (shape(y), us)
    elif len(us) == 2:
        nx, ny = us
        if x is None and y is None:
            if indexing == 'ij':
            ...
            if indexing == 'ij':
                assert (
                    shape(x) == (nx, ny) or
                    shape(x) == (nx, 1) or
                    shape(x) == (nx,)), '_check_xyuv: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny), (nx, 1), (nx,))
                assert (
                    shape(y) == (nx, ny) or
                    shape(y) == (1, ny) or
                    shape(y) == (ny,)
                ), '_check_xyuv: y has shape %s, expected %s, %s, or %s' % (shape(y), (nx, ny), (1, ny), (ny,))
            else:
                assert (
                    shape(x) == (nx, ny) or
                    shape(x) == (1, ny) or
                    shape(x) == (ny,)
                ), '_check_xyuv: x has shape %s, expected %s, %s, or %s' % (shape(x), (nx, ny), (1, ny), (ny,))
                assert (
                    shape(y) == (nx, ny) or
                    shape(y) == (nx, 1) or
                    shape(y) == (nx,)
                ), '_check_xyuv: y has shape %s, expected %s, %s, or %s' % (shape(y), (nx, ny), (nx, 1), (nx,))
    else:
...
def _check_xyzuvw(*args, **kwargs):
    ...
    if len(us) == 1:
        if x is None and y is None:
            ...
            assert shape(x) == us, '_check_xyuv: x has shape %s, expected %s' % (shape(x), us)
            assert shape(y) == us, '_check_xyuv: y has shape %s, expected %s' % (shape(y), us)
        assert shape(z) == us, '_check_xyuv: z has shape %s, expected %s' % (shape(z), us)
    elif len(us) == 2:
        nx, ny = us
        if x is None and y is None:
        ...
        assert shape(z) == us, '_check_xyzuvw: z, u, v, and w must be of same shape'
    elif len(us) == 3:
        nx, ny, nz = us
        if x is None and y is None:
            if indexing == 'ij':
            ...
            if indexing == 'ij':
                assert (
                    ...
                    shape(x) == (nx, 1, 1) or
                    shape(x) == (nx,)
                ), '_check_xyzuvw: x has shape %s, expected %s, %s, or %s' % (shape(x), us, (nx, 1, 1), (nx,))
                assert (
                    shape(y) == us or
                    shape(y) == (1, ny, 1) or
                    shape(y) == (ny,)
                ), '_check_xyzuvw: y has shape %s, expected %s, %s, or %s' % (shape(y), us, (1, ny, 1), (ny,))
            else:
                assert (
                    shape(x) == us or
                    shape(x) == (1, ny, 1) or
                    shape(x) == (ny,)
                ), '_check_xyzuvw: x has shape %s, expected %s, %s, or %s' % (shape(x), us, (1, ny, 1), (ny,))
                assert (
                    shape(y) == us or
                    shape(y) == (nx, 1, 1) or
                    shape(y) == (nx,)
                ), '_check_xyzuvw: y has shape %s, expected %s, %s, or %s' % (shape(y), us, (nx, 1, 1), (nx,))
        assert (
            shape(z) == us or
            shape(z) == (1, 1, nz) or
            shape(z) == (nz,)
        ), '_check_xyzuvw: z has shape %s, expected %s, %s, or %s' % (shape(z), us, (1, 1, nz), (nz,))
    else:
